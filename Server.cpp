		...
		if (nTotal > 0) {
			...
			if (FD_ISSET(sessionSocket, &rfds)) {
				...
				if (rtn > 0) {
					...
					// 响应体数据
					std::string responseData = "<html><body><h1>Hello, World!</h1></body></html>";
					// 构建 HTTP 响应头
					std::string responseHeader = "HTTP/1.1 200 OK\nContent-Type: text/html\nContent-Length: " + std::to_string(responseData.size()) + "\n\n";
					send(sessionSocket, responseHeader.c_str(), responseHeader.size(), 0);
					send(sessionSocket, responseData.c_str(), responseData.size(), 0);
